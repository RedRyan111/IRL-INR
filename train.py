...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(config["epochs"])):
        ...
        for i, dict in tqdm(enumerate(train_dataloader)):
            ...
            rotated_images = transforms.RandomRotation(180)(
                images.view(-1, config["feature_dim"], config["resolution"], config["resolution"]))
            ...
            if config["dataset"] in ["wm811k", "plankton", "5hdb"]:
                ...
                for k in range(b):
                    ...
                    rotated_image = rotated_image.reshape(config["resolution"], config["resolution"],
                                                          config["feature_dim"])
        ...
        if epoch % 1 == 0:
            for dict in val_dataloader:
                ...
                rotated_images = torch.empty(16 * 8, config["feature_dim"], config["resolution"],
                                             config["resolution"]).to(device)
                a = 0
                for j in range(16):
                    for i in range(8):
                ...
                save_image(
                    generated_images.view(16 * 8, config["feature_dim"], config["resolution"], config["resolution"]),
                    directory + '/valset_{}th_rotated_output.png'.format(epoch + 1))
                break
